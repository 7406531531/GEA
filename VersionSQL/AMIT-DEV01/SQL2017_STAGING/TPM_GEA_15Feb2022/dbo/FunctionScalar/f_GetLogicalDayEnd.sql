/****** Object:  Function [dbo].[f_GetLogicalDayEnd]    Committed by VersionSQL https://www.versionsql.com ******/

/**************************************************************************************
DR0115 - Altered By KarthikG on 16-Jun-2008 :: "and running = 1" has been added in outer query
***************************************************************************************/
Create     Function [dbo].[f_GetLogicalDayEnd](@FromDate datetime)
RETURNS datetime
AS
BEGIN
...
SELECT @DT = ISNULL(Dateadd(DAY,ToDay,(convert(datetime, @StartDate + ' ' + CAST(datePart(hh,ToTime) AS nvarchar(2)) + ':' + CAST(datePart(mi,ToTime) as nvarchar(2))+ ':' + CAST(datePart(ss,ToTime) as nvarchar(2))))),(@StartDate + ' 00:00:00'))
	 ...
	 if @st>=@FromDate
begin
set @dt=dateadd(day,0,@st)
--set @st=dateadd(day,1,@st)
End
if @dt<@FromDate
begin
set @dt=dateadd(day,-1,@st)
end
