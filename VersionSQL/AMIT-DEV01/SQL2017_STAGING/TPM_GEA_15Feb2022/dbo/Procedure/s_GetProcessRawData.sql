...
DECLARE @Splstring1 As decimal(18,3) --GEA --Special string to store PalletCount(Type 1) or ProgramId(Type3) or SpindleDir(Type4) Or ToolNo(Type 5)Or PAlarmNo(Type 6)
...
DECLARE SD_CUR CURSOR FOR SELECT Slno,DataType,IPAddress,Mc,Comp,Opn,Opr,Splstring1,Sttime,Ndtime,SplString2,Status,WorkOrderNo,Splstring3,Splstring4,Splstring5,Splstring6 FROM #SD_RawData
...
FETCH NEXT FROM SD_CUR INTO @Slno,@DataType,@IPAddress, @Mc,@Comp,@Opn,@Opr,@Splstring1,@Sttime,@Ndtime,@SplString2,@Status,@WorkOrderNo,@Splstring3,@WearOffSetNumber,@MeasureDimension,@CorrectionValue
...
Select @COPSlNo=Max(SlNo) From componentoperationpricing where componentid=@CI_ComponentID And interfaceid=@Opn
...
if @Act_LoadUnload<=5 and (isnumeric(@Splstring1)>=1 and isnull(@Splstring1,5)<>5) and @Splstring1<>0
...
if @Act_LoadUnload<=5 and (datediff(ss,@LastStarttime,@TempNdTime)-@TpmStdLoadunload)>0 and ((Select count(*) from autodata where mc=@mc and sttime=@LastStarttime and datatype=2)=1)
...
ELSE if (@Act_LoadUnload>0 and @Act_LoadUnload<=5) and (datediff(ss,@LastStarttime,@TempNdTime)-@TpmStdLoadunload)<0 and ((Select count(*) from autodata where mc=@mc and sttime=@LastStarttime and datatype=2)=1)
...
Insert into autodata (datatype,mc,comp,opn,opr,dcode,stdate,sttime,nddate,ndtime,cycletime,loadunload,msttime,WorkOrderNumber,compslno) values
...
insert into autodata (datatype,mc,comp,opn,opr,dcode,stdate,sttime,nddate,ndtime,cycletime,loadunload,msttime,WorkOrderNumber,compslno)
...
Insert into autodata (datatype,mc,comp,opn,opr,stdate,sttime,nddate,ndtime,cycletime,loadunload,msttime,PartsCount,WorkOrderNumber,compslno) values
...
SELECT 2, @Mc , @Comp , @Opn, @Opr ,@ICDdcode,@ICDstdate,@ICDsttime,@ICDnddate,@ICDndtime,@ICDcycletime,Datediff(s,@ICDsttime,@ICDndtime),@ICDsttime,@WorkOrderNo,@Splstring3
...
Insert into SPCAutodata([mc],[comp],[opn],[opr],[Dimension],[Value],[Timestamp],[BatchTS],WearOffSetNumber,MeasureDimension,CorrectionValue)



Select @MC,@COMP,@OPN,@OPR,@splstring3,@SPLSTRING2,@sttime,@sttime,@WearOffSetNumber,@MeasureDimension,@CorrectionValue
...
Select @MC,@COMP,@OPN,@OPR,@splstring3,@SPLSTRING2,@sttime,@BatchTS,@WearOffSetNumber,@MeasureDimension,@CorrectionValue
