...
--[dbo].[TPMTrakMobile_ViewShift&HourwiseSummary]  '2019-12-22',''
**********************************************************************************/
...
--	Update #Machine Set McStatus = 'Stopped',Color='Yellow' where  McStatus='0'
	...
	Select PlantCode,PlantID,MCInterface,MachineId,RunningPart,Round(Partscount,3) as Partscount,dbo.f_FormatTime(Stoppages,'hh:mm') as Stoppages,McStatus,color,MachineMTB,LastRecord,Reason from #Machine
--End
...
SELECT PlantCode,PlantID,MCInterface,Machineid,DisplayName,DisplayOrder,NavID,case when Columnname not in ('cycles','Powerontime','Operatingtime','Cuttingtime') then dbo.f_FormatTime(DayValue,'hh:mm') else cast(DayValue as nvarchar(50)) end
,case when Columnname not in ('cycles','Powerontime','Operatingtime','Cuttingtime') then dbo.f_FormatTime(SUM(ColumnValue),'hh:mm') else cast(SUM(ColumnValue) as nvarchar(50)) end  from #finaldata 
...
(SELECT PlantCode,PlantID,MCInterface,Machineid,DisplayName,DisplayOrder,NavID,case when Columnname not in ('cycles','Powerontime','Operatingtime','Cuttingtime') then dbo.f_FormatTime(SUM(ColumnValue),'hh:mm') else cast(SUM(ColumnValue) as nvarchar(50)) end as Columnname2 from #finaldata  
...
Begin
	...
	(SELECT PlantCode,PlantID,MCInterface,Machineid,DisplayName,DisplayOrder,NavID,case when Columnname not in ('cycles','Powerontime','Operatingtime','Cuttingtime') then dbo.f_FormatTime(SUM(ColumnValue),'hh:mm') else cast(SUM(ColumnValue) as nvarchar(50)) end AS Columnname3 from #finaldata  
