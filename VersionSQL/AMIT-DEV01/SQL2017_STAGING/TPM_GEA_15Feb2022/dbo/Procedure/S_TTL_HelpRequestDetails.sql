...
--[dbo].[S_TTL_HelpRequestDetails] '2020-08-31','','','','''WFL M50 Mill Turn''','',''
...
ACKTime float,
FIXTime float,
RemarksForRaised nvarchar(4000),
RemarksForClosed nvarchar(4000)
)
...
Update #ACK_request set RemarksforRaised=T2.Remarks From (
Select H.plantid,H.machineid,H.helpcode,H.Starttime as [Raised TS],H.Remarks from helpcodedetails H
inner join #ACK_request AK on H.plantid=AK.Pinterface and H.Machineid=AK.Minterface and H.HelpCode=AK.Eventinterface and H.StartTime=AK.[Raised TS]
where (H.Action1='01')
...
Update #ACK_request set RemarksForClosed=T2.Remarks From (
Select H.plantid,H.machineid,H.helpcode,H.Starttime as [Raised TS],H.Remarks,H.EndTime from helpcodedetails H
inner join #ACK_request AK on H.plantid=AK.Pinterface and H.Machineid=AK.Minterface and H.HelpCode=AK.Eventinterface and H.StartTime=AK.[Raised TS] and H.EndTime=AK.[Close TS]
where (H.Action1='01' and H.Action2='03')
)as T2 inner join #ACK_request r on T2.plantid=r.Pinterface and T2.Machineid=r.Minterface and T2.HelpCode=r.Eventinterface
and r.[Raised TS] = t2.[Raised TS]
...
If @param = ''
BEGIN
Select [PSerial Number], [Request Desc], [Raised TS], [SMS Sent TS], [SMS Sent To], [Ack TS], [Close TS], [Close SMS Sent TS], [Close SMS Sent To],RemarksForRaised,RemarksForClosed
