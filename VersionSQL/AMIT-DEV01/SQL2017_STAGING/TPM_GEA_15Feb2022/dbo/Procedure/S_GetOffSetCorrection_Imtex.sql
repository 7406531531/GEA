...
BEGIN
	...
	BEGIN
		...
		where (Machineinformation.machineid = @MachineId or ISNULL(@MachineId,'')='') and (timestamp >= @StartTime and timestamp <= @EndTime)
		order by TimeStamp desc
	END
	ELSE
	BEGIN
...
if  ((select ValueInText FROM ShopDefaults WHERE Parameter = 'SPCCompOpnSet') = 'SPCMaster')
	BEGIN
		SELECT ROW_NUMBER() OVER (ORDER BY A.TimeStamp desc) AS CycleNo, machineinformation.machineid, S.ShiftDate,S.ShiftName,A.TimeStamp as MeasuredTime, A.Dimension,A.WearOffsetNumber,
		...
		where (Machineinformation.machineid = @MachineId or ISNULL(@MachineId,'')='') and (timestamp >= S.ShiftStart and timestamp <= S.ShiftEnd) and (S.ShiftName = @ShiftName or ISNULL(@ShiftName, '')='') 
		order by TimeStamp desc
	END
	ELSE
	BEGIN
