...
(Select T.Helpcode,T.HelpDescription,T.RowID,#HelpRequest.Remarks from #HelpRequest 
Inner Join (Select Helpcode,HelpDescription,RowID from #HelpRequest where RowID IS NOT NULL AND Rowid>0)T
on T.Rowid=#HelpRequest.Rowid
)T2 INNER JOIN #HelpRequest ON #HelpRequest.Helpcode=T2.Helpcode
