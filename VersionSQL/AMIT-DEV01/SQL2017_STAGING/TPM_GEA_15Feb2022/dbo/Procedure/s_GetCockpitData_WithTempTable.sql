...
s_GetCockpitData_WithTempTable '2020-03-01 09:00:00 AM','2020-03-10 02:30:00 AM','',''  
****************************************************************************************************/  
...
update #CockPitData set AvgAE= case when (UT+DT-ML)>0 then (UT/(UT+DT-ML)) else 0 end,AvgPE=case when ut>0 then (CN1/ut) else 0 end from
(select ISNULL(SUM(UtilisedTime),0) as ut,ISNULL(SUM(DownTime),0) as Dt,ISNULL(SUM(ManagementLoss),0) as ML,SUM(CN) as CN1 from #CockPitData)T

--select  dbo.f_FormatTime(ISNULL(SUM(UtilisedTime),0),@timeformat) as ut,SUM(UtilisedTime) UtilisedTime,
--dbo.f_FormatTime(ISNULL(SUM(DownTime),0),@timeformat) as Dt,SUM(DownTime) DownTime,
--ISNULL(SUM(ManagementLoss),0) as ML,SUM(CN) as CN1 from #CockPitData

update #CockPitData set AvgOE=(AvgAE*AvgPE*100),AvgAE=AvgAE*100,AvgPE=AvgPE*100
