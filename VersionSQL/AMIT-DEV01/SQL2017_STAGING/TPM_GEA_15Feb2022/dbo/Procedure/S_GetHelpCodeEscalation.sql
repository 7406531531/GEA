...
from (
select distinct HD.Machineid,HD.LastCallTime as raisedtime,HD.HelpCode,'TPM-Trak Msg From Machine  ' + HD.Machineid + ' : ' + isnull(HR.Message,'') + ' ' + Convert(nvarchar(20),HD.LastCallTime,100) as Msg from #FinalHelpCode HD
left join HelpRequestRule HR on HD.Plantid=HR.Plantid and HD.Machineid=HR.Machineid and HD.HelpCode=HR.HelpCode and HR.Action='04'
)t inner join #FinalHelpCode on 
#FinalHelpCode.MachineID = t.MachineID and #FinalHelpCode.LastCallTime = t.raisedtime and #FinalHelpCode.Helpcode = t.Helpcode


  select distinct Plantid,Machineid,HelpDescription,HelpCode,LastCallTime,MobileNo,MessageInfo,'04' as ActionNo  into #FinalOutput1 from #FinalHelpCode
 order by Machineid,HelpCode


 Insert into Messagehistory(StartTime,Requestedtime,Msgstatus,MobileNo,Message,Machineid,ActionNo,HelpCode)
  select distinct LastCallTime,LastCallTime,0,MobileNo,MessageInfo, Machineid,ActionNo,HelpCode from #FinalOutput1 A1
  where not exists(select * from MessageHistory A2 where A1.Machineid=A2.MachineID and A1.LastCallTime=A2.SendTime and A1.HelpCode=A2.HelpCode and A1.ActionNo=A2.ActionNo)
