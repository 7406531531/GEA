...
if isnull(@MO,'')<> ''
Begin
	SELECT @StartTime = MIN(msttime), @Endtime = Max(ndtime) from autodata where workorderNumber = @MO group by workorderNumber
End
...
Begin
	...
	SET @strMO = ' AND MD.MONumber = N''' + @MO + ''''
End
...
Select @strsql = @strsql + @strMachine + @strPlantID + @strMO
PRINT @strSql
EXEC(@strSql)
