...
SET @strsql = 'Select distinct pm.Plantid from plantmachine pm inner join machineinformation mi
	on   pm.machineid=mi.machineid and  (DNCTransferEnabled = 1 OR EthernetEnabled = 1) order by pm.plantid '
...
BEGIN
SET @strsql = ''
SET @strsql = 'select Machineid from machineinformation where (DNCTransferEnabled = 1 OR EthernetEnabled = 1) order by machineid '
SET @strsql = @strsql + @order
EXEC (@strsql)
END
...
INNER JOIN machineinformation mi
  ON pm.Machineid = mi.Machineid
    AND (DNCTransferEnabled = 1 OR EthernetEnabled = 1)
