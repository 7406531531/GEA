/****** Object:  Procedure [dbo].[S_GetCategorywiseDownandFrequency]    Committed by VersionSQL https://www.versionsql.com ******/
...
-- =============================================
---[dbo].[S_GetCategorywiseDownandFrequency] '','','Wipro-MH Plant','ECONO-2','''Operator''','''START UP LOSSES''',''
---[dbo].[S_GetCategorywiseDownandFrequency] '2014-03-01 06:00:00','2014-03-15 06:00:00','Wipro - TH Plant','','','',''


CREATE PROCEDURE [dbo].[S_GetCategorywiseDownandFrequency]
...
BEGIN
			...
			(
			SELECT mc,count(mc)as dwnfrq,sum(loadunload)as down,downcodeinformation.catagory as catagory
			...
			group by autodata.mc,downcodeinformation.catagory 
			)as t2 inner join #DownTimeData on t2.mc=#DownTimeData.McInterfaceid and t2.catagory=#DownTimeData.DownCategory


			--TYPE 2
			UPDATE #DownTimeData SET downtime = isnull(DownTime,0) + isnull(t2.down,0) , DownFreq = isNull(downfreq,0) + isNull(t2.dwnfrq,0) FROM
			(
			SELECT mc,count(mc)as dwnfrq,sum(DateDiff(second, @StartTime, ndtime))as down,downcodeinformation.catagory as catagory
			from autodata INNER JOIN machineinformation ON autodata.mc = machineinformation.InterfaceID 
			Left Outer Join PlantMachine ON PlantMachine.MachineID=machineinformation.MachineID
			Inner JOIN downcodeinformation ON autodata.dcode = downcodeinformation.interfaceid
			inner join Downcategoryinformation on Downcategoryinformation.Downcategory=downcodeinformation.Catagory
			...
			UPDATE #DownTimeData SET downtime = isnull(DownTime,0) + isnull(t2.down,0),DownFreq = isNull(downfreq,0) + isNull(t2.dwnfrq,0) FROM
			(
			SELECT mc,count(mc)as dwnfrq,sum(DateDiff(second, mstTime, @Endtime))as down,downcodeinformation.catagory as catagory
			from autodata INNER JOIN machineinformation ON autodata.mc = machineinformation.InterfaceID 
			Left Outer Join PlantMachine ON PlantMachine.MachineID=machineinformation.MachineID
			Inner JOIN downcodeinformation ON autodata.dcode = downcodeinformation.interfaceid
			inner join Downcategoryinformation on Downcategoryinformation.Downcategory=downcodeinformation.Catagory
			...
			UPDATE #DownTimeData SET downtime = isnull(DownTime,0) + isnull(t2.down,0),DownFreq = isNull(downfreq,0) + isNull(t2.dwnfrq,0) FROM
			(
			SELECT mc,count(mc)as dwnfrq,sum(DateDiff(second, @StartTime,@EndTime))as down,downcodeinformation.catagory as catagory
			from autodata INNER JOIN machineinformation ON autodata.mc = machineinformation.InterfaceID 
			Left Outer Join PlantMachine ON PlantMachine.MachineID=machineinformation.MachineID 
			Inner JOIN downcodeinformation ON autodata.dcode = downcodeinformation.interfaceid
			inner join Downcategoryinformation on Downcategoryinformation.Downcategory=downcodeinformation.Catagory
			...
			END) AS LOSS,downcodeinformation.catagory as catagory from autodata 
			...
			END)loss, downcodeinformation.catagory as catagory
			from autodata INNER JOIN machineinformation ON autodata.mc = machineinformation.InterfaceID 
			Left Outer Join PlantMachine ON PlantMachine.MachineID=machineinformation.MachineID 
			Inner JOIN downcodeinformation ON autodata.dcode = downcodeinformation.interfaceid
			inner join Downcategoryinformation on Downcategoryinformation.Downcategory=downcodeinformation.Catagory
...
BEGIN
			...
			from
			(select mc,downcodeinformation.catagory as catagory,count(mc)as dwnfrq,sum(
					CASE
			...
			(
				SELECT autodata.MC, downcodeinformation.catagory as catagory,SUM
				   (CASE
				...
				group by autodata.mc,downcodeinformation.catagory
			) as TT inner join #DownTimeData on TT.mc=#DownTimeData.McInterfaceid and TT.catagory=#DownTimeData.DownCategory
			...
			UPDATE #DownTimeData SET ManagementLoss = isnull(ManagementLoss,0) + isNull(t4.Mloss,0),MLDown=isNull(MLDown,0)+isNull(t4.Dloss,0)
					from
					(select T3.mc,T3.Catagory,sum(T3.Mloss) as Mloss,sum(T3.Dloss) as Dloss from (
					select   t1.id,T1.mc,T1.Threshold,T1.Catagory,
					...
					(   select id,mc,D.threshold,D.catagory,
					...
					left outer join
					(SELECT autodata.id,downcodeinformation.catagory,
							   sum(CASE
							WHEN autodata.sttime >= T.StartTime  AND autodata.ndtime <=T.EndTime  THEN (autodata.loadunload)
							WHEN ( autodata.sttime < T.StartTime  AND autodata.ndtime <= T.EndTime  AND autodata.ndtime > T.StartTime ) THEN DateDiff(second,T.StartTime,autodata.ndtime)
							WHEN ( autodata.sttime >= T.StartTime   AND autodata.sttime <T.EndTime  AND autodata.ndtime > T.EndTime  ) THEN DateDiff(second,autodata.sttime,T.EndTime )
						...
						WHERE autodata.DataType=2 AND T.MachineInterface=autodata.mc AND
							...
							AND (downcodeinformation.availeffy = 1) 
							group  by autodata.id,downcodeinformation.catagory) as T2 on T1.id=T2.id ) as T3  group by T3.mc,T3.Catagory
					) as t4 inner join #DownTimeData on t4.mc = #DownTimeData.McInterfaceid AND #DownTimeData.DownCategory=T4.catagory
...
(Select McInterfaceid as mc,DownCategory as catagory,Downtime,Downfreq ,Managementloss as Mloss,MLDown,PDT from #DownTimeData)T1
INNER JOIN #FinalDownData ON T1.mc = #FinalDownData.McInterfaceid AND #FinalDownData.DownCategory=T1.catagory
...
select 	PlantID,Machineid,DownCategory,DownID,Round(Downtime/60,2) as Downtime,Managementloss,DownFreq,Round(Bookingtime/60,2) as Bookingtime,
...
round(TotalDownLossMachinewise/60,2) as TotalDownLossMachinewise,TotalDownFreqMachinewise from #FinalDownData
