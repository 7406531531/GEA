...
-- [s_GetMando_Reports] '2020-06-10 06:00:00','2020-06-11 06:00:00','','','','1','2'
*/
...
--		select @strXmachine = ' and ( EX.MachineID = ''' + @Machine + ''')'
	...
	Update #FINALOUTPUT set RowValue = t1.Tcount from 
	--(select CAST(CEILING(CAST(sum(autodata.partscount)AS Float)/ISNULL(componentoperationpricing.SubOperations,1)) AS INTEGER) as Tcount,
	(select CAST(sum(autodata.partscount)AS Float)/ISNULL(componentoperationpricing.SubOperations,1) as Tcount,
	...
	BEGIN
		...
		from
		( select T.Shiftst as hrst ,#FINALOUTPUT.MachineID as mach,
		--CAST(CEILING(CAST(sum(autodata.partscount)AS Float)/ISNULL(O.SubOperations,1)) AS INTEGER) as compcnt ,
		CAST(sum(autodata.partscount)AS Float)/ISNULL(O.SubOperations,1) as compcnt ,
	...
	(select machine,component,operation,operator,
	--sum(convert(int,rowvalue)) as TotOutput from #FinalOutput group by machine,component,operation,operator)
	sum(convert(float,rowvalue)) as TotOutput from #FinalOutput group by machine,component,operation,operator)
