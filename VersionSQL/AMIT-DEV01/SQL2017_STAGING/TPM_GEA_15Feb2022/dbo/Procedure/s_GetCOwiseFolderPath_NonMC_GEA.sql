/****** Object:  Procedure [dbo].[s_GetCOwiseFolderPath_NonMC_GEA]    Committed by VersionSQL https://www.versionsql.com ******/

--[dbo].[s_GetCOwiseFolderPath_GEA]'MCV 320','ATH-16-16','7'
CREATE PROCEDURE [dbo].[s_GetCOwiseFolderPath_NonMC_GEA]
...
select @MOPath=''
select @MOPath=folderpath,@MOPathExt =FileExtension from  FolderPathDefinition_NonMachining where FolderType='MOPath'
select @ControlPath=folderpath,@ControlPathExt=FileExtension from FolderPathDefinition_NonMachining where FolderType='ControlPath'
select @ProgramNumber=folderpath,@ProgramNumberExt=FileExtension from FolderPathDefinition_NonMachining where FolderType='ProgramPath'
select @DrawingPath=folderpath,@DrawingPathExt=FileExtension from FolderPathDefinition_NonMachining where FolderType='DrawingPath'
select @MacSel = isnull(folderpath,0) from FolderPathDefinition_NonMachining where FolderType='MachineSelected'
select @OperationPath =folderpath,@OperationPathExt=FileExtension from  FolderPathDefinition_NonMachining where FolderType='OperationPath'
...
If @MacSel = 1
BEGIN
	SELECT @MOPath=@MOPath + '\' + RTRIM(lTRIM(@machineid)) +'\'+ RTRIM(lTRIM(@Componentid))
	SELECT @DrawingPath=@DrawingPath + '\' + RTRIM(lTRIM(@machineid)) +'\'+ RTRIM(lTRIM(@Componentid))
	SELECT @ProgramNumber=@ProgramNumber + '\' + RTRIM(lTRIM(@machineid)) +'\'+ RTRIM(lTRIM(@Componentid))
	SELECT @ControlPath=@ControlPath + '\' + RTRIM(lTRIM(@machineid)) +'\'+ RTRIM(lTRIM(@Componentid))
	SELECT @OperationPath=@OperationPath + '\' + RTRIM(lTRIM(@machineid)) +'\'+ RTRIM(lTRIM(@Componentid))
END
else
BEGIN
	SELECT @MOPath=@MOPath + '\' + RTRIM(lTRIM(@Componentid))
	SELECT top 1 @DrawingPath=@DrawingPath +'\'+RTRIM(lTRIM(@Componentid))
	SELECT @ProgramNumber=@ProgramNumber + '\' +RTRIM(lTRIM(@Componentid))
	SELECT @ControlPath=@ControlPath+'\'+RTRIM(lTRIM(@Componentid))
	SELECT @OperationPath=@OperationPath+'\'+RTRIM(lTRIM(@Componentid))
END
