...
/*
[dbo].[TPMTrakMobile_InsertShift&HourwiseSummary]     '2021-08-13 07:01:00'
*/
...
/******************************************************* SHIFTWISE SUMMARY *********************************************************/
		...
		where (Rawdata.sttime>A.Endtime and Rawdata.sttime>=S.[From time] and rawdata.sttime<=S.[To Time]) and rawdata.datatype=11 and rawdata.[status] in (1,15)
		group by rawdata.mc,S.[From time]) t1   
		...
		where (Rawdata.sttime>A.Endtime and Rawdata.sttime>=fd.[From time] and rawdata.sttime<=fd.[To Time]) and rawdata.datatype=11   and rawdata.[status] in (1,15)
		order by rawdata.mc  
		...
		where rawdata.datatype in(40,41,42) and rawdata.[status] in (1,15)
		and (rawdata.sttime>M.sttime and rawdata.sttime>=M.shiftstart and ISNULL(Rawdata.ndtime,Rawdata.sttime)<=M.ShiftEnd) 
		...
		inner join (select mc,max(slno) as slno,S.[From time] from rawdata WITH (NOLOCK)   
		inner join #ShiftwiseSummary S on RawData.mc=S.MachineInterface 
		where Rawdata.sttime<S.[To Time] and rawdata.datatype in(11,1,2,22,42) and rawdata.[status] in (1,15)
		group by mc,S.[From time]) t1 on t1.mc=rawdata.mc and t1.slno=rawdata.slno  
	   ...
	   ---------------------------------------------------- DAYWISE RUNNINGCYCLE Calculation---------------------------------------------------------------
			...
			inner join Autodata_maxtime A on rawdata.mc=A.machineid where (Rawdata.sttime>A.Endtime and Rawdata.sttime<@currtime) and rawdata.datatype=11 and rawdata.[status] in (1,15)
			group by mc  ) t1   
			on t1.mc=rawdata.mc and t1.slno=rawdata.slno  
			inner join Autodata_maxtime A on rawdata.mc=A.machineid  
			right outer join (select distinct machineid,MachineInterface from #ShiftwiseSummary) fd on fd.MachineInterface = rawdata.mc  
			where (Rawdata.sttime>A.Endtime and Rawdata.sttime<@currtime) and rawdata.datatype=11 and rawdata.[status] in (1,15)  
			order by rawdata.mc  
			...
			inner join #machineRunningStatus M on M.MachineInterface=rawdata.mc  
			where rawdata.datatype in(40,41,42) and (rawdata.sttime>M.sttime and ISNULL(Rawdata.ndtime,Rawdata.sttime)<@currtime)  and rawdata.[status] in (1,15)
			group by rawdata.mc  
			)T1  inner join rawdata on rawdata.slno=t1.slno  
			inner join #machineRunningStatus M on M.MachineInterface=rawdata.mc  
