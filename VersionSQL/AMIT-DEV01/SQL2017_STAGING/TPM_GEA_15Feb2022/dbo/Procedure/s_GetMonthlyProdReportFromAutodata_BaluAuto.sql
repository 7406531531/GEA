/****** Object:  Procedure [dbo].[s_GetMonthlyProdReportFromAutodata_BaluAuto]    Committed by VersionSQL https://www.versionsql.com ******/
...
[s_GetMonthlyProdReportFromAutodata_BaluAuto] '2019-14-14','A','PKH HBM-02'
[s_GetMonthlyProdReportFromAutodata_BaluAuto] '2019-05-13','','CNC-01'
exec s_GetMonthlyProdReportFromAutodata_BaluAuto '2019-05-31','FIRST ','CNC-01'
exec s_GetMonthlyProdReportFromAutodata_BaluAuto '2019-05-02','FIRST','CNC-01','','','','2019-05-04','Shift'
exec s_GetMonthlyProdReportFromAutodata_BaluAuto '2020-03-10',''
**************************************************************************************************/
CREATE PROCEDURE [dbo].[s_GetMonthlyProdReportFromAutodata_BaluAuto]
	@Date datetime,
	@ShiftIn nvarchar(20) = '',
...
(
	...
	OperationCount float, 
	PartCycleTime Float,
	cyclefficiency float,
	LoadUnloadefficiency float
)
...
set @MinLuLR=isnull((select top 1 valueinint from Shopdefaults where parameter='MinLUforLR'),0)
	 ...
	 -- SELECT DISTINCT Machine ,0,MachineInt,0,0,0,0,0,0,0,Shdate,
		...
		select @strsql = @strsql + ' SELECT  distinct machineinformation.machineid, componentinformation.componentid, 
		 ...
		 (CAST(Sum(autodata.partscount)AS Float)/ISNULL(ComponentOperationPricing.SubOperations,1)) as PCount ,
		 Pdate, Shift, ShiftStart, ShiftEnd ,machineinformation.interfaceid,componentinformation.interfaceid,componentoperationpricing.interfaceid
		 FROM  (Select distinct mc,comp,opn,opr,Pdate, Shift, ShiftStart, ShiftEnd,cycletime,loadunload,partscount
								  from #T_autodata 
									cross join #ShiftTimeDetails 
									 where 
									--(( sttime >= shiftstart and ndtime <= shiftend ) OR 
									--( sttime < shiftstart and ndtime > shiftend ) OR 
									--( sttime < shiftstart and ndtime > shiftstart and ndtime<=shiftend ) OR 
									--( sttime >= shiftstart and ndtime > shiftend and sttime<shiftend )) 	
									 (( sttime >= shiftstart and ndtime <= shiftend ) OR 
									( sttime < shiftstart and ndtime > shiftstart and ndtime<=shiftend ))
									 and datatype=1  AND (partscount > 0 )		
									)  autodata INNER JOIN  machineinformation ON autodata.mc = machineinformation.InterfaceID INNER JOIN  
		 componentinformation ON autodata.comp = componentinformation.InterfaceID  INNER JOIN 
		 componentoperationpricing ON (autodata.opn = componentoperationpricing.InterfaceID)
		 and componentoperationpricing.machineid=machineinformation.machineid 
		 AND (componentinformation.componentid = componentoperationpricing.componentid) 
		...
		BEGIN
			...
			UPDATE #Target SET OperationCount=ISNULL(OperationCount,0)- isnull(t2.PlanCt,0)
				...
				((CAST(Sum(ISNULL(PartsCount,1)) AS Float)/ISNULL(Componentoperationpricing.SubOperations,1))) as PlanCt,
			 	Componentinformation.componentid as compid,componentoperationpricing.Operationno as opnno 
...
---------------------------------CN------------------------------------------------------------------	
	...
	UPDATE #Finaldata SET CN = isnull(CN,0) + isNull(t2.C1N1,0)
		from
		(select mc,
		  SUM((componentoperationpricing.cycletime/ISNULL(ComponentOperationPricing.SubOperations,1))*autodata.partscount) C1N1,Sdate as date1,ShftStart as ShiftStart
		   from     (
					SELECT mc,Comp,opn,Sum(partscount) partscount,ShftStart,ShftEnd ,Sdate
				     FROM #T_autodata  inner join (select distinct MachineInterface,ShftStart,ShftEnd,Sdate from #Finaldata) S on mc=S.MachineInterface
							   where (ndtime>=S.ShftStart) and (ndtime<=S.ShftEnd)
								and (datatype=1)
								group by mc,Comp,opn,ShftStart,ShftEnd ,Sdate
							   ) autodata  
		...
		inner join machineinformation on machineinformation.interfaceid=autodata.mc
		and componentoperationpricing.machineid=machineinformation.machineid
		  group by autodata.mc,Sdate,ShftStart
		) as t2 inner join #Finaldata on t2.mc = #Finaldata.machineinterface
		and t2.date1=#Finaldata.Sdate and t2.ShiftStart=#Finaldata.ShftStart
...
set TargetCycleTime = S.CycleTime,
 ...
 CASE WHEN ISNULL(SUM(OperationCount),0) <> 0 THEN AVG(PartCycleTime)/SUM(OperationCount) END PartCycleTime
 from #Target
...
--select DISTINCT 
--		ROUND(AVG(#Finaldata.ProductionEfficiency),0) ProductionEfficiency,
...
--	    from  #Finaldata 
--		WHERE Shiftid <> 3 AND  (ISNULL(UtilisedTime,0) <> 0 OR ISNULL(DownTime,0) <> 0) 
--		GROUP BY #Finaldata.Sdate,#Finaldata.Shiftid,#Finaldata.ShiftName
--		ORDER BY #Finaldata.Sdate,#Finaldata.Shiftid

--select * from #Finaldata 
--WHERE (ISNULL(UtilisedTime,0) <> 0 OR ISNULL(DownTime,0) <> 0) 
----AND Shiftid <> 3   
--order by Shiftid,Sdate,MachineID

Insert into #Summary(PlannedProductionTime,DownTime,UtilisedTime,CN,ManagementLoss,TargetCycleTime,LoadUnloadTime,PartCycleTime,OperationCount,
cyclefficiency,LoadUnloadefficiency,D1,D2,D3,D4,D5,D6,D7,D8,D9,D10,D11,D12,D13,Others,TotalLoss,ActLoss,LossErr,TargetCount,components,RejCount,QuantityOk,
sdate,Shiftid,ShiftName)
select
		SUM(#Finaldata.PlannedProductionTime),
		SUM(#Finaldata.DownTime),
		SUM(#Finaldata.UtilisedTime), 
		SUM(CN), 
		SUM(#Finaldata.ManagementLoss), 
		isnull(SUM(#Finaldata.TargetCycleTime),0),
		isnull(SUM(#Finaldata.LoadUnloadTime),0),
		isnull(SUM(#Finaldata.PartCycleTime),0),
		isnull(Round(SUM(#Finaldata.OperationCount),2),0)as OperationCount, 
		cyclefficiency =
		SUM(CASE
		   when ( isnull(#Finaldata.TargetCycleTime,0) > 0 and
		...
		END),
		isnull(SUM(#Finaldata.D1),0),
		isnull(SUM(#Finaldata.D2),0),
		isnull(SUM(#Finaldata.D3),0),
		isnull(SUM(#Finaldata.D4),0),
		isnull(SUM(#Finaldata.D5),0),
		isnull(SUM(#Finaldata.D6),0),
		isnull(SUM(#Finaldata.D7),0),
		isnull(SUM(#Finaldata.D8),0),
		isnull(SUM(#Finaldata.D9),0),
		isnull(SUM(#Finaldata.D10),0),
		isnull(SUM(#Finaldata.D11),0),
		isnull(SUM(#Finaldata.D12),0),
		isnull(SUM(#Finaldata.D13),0),isnull(SUM(#Finaldata.Others),0),
		isnull(SUM(#Finaldata.TotalLoss),0),
		isnull(SUM(#Finaldata.ActLoss),0),
		isnull(SUM(#Finaldata.LossErr),0),
		isnull(SUM(#Finaldata.TargetCount),0) as QuantityPlanned, 
		isnull(SUM(#Finaldata.components),0) as QuantityProduced,
		isnull(SUM(#Finaldata.RejCount),0) as QuantityRejected,
		isnull(SUM(#Finaldata.QuantityOk),0) as QuantityOk,
	    ...
	    from  #Finaldata 
		WHERE Shiftid <> 3 
		--AND  (ISNULL(UtilisedTime,0) <> 0 OR ISNULL(DownTime,0) <> 0) 
		GROUP BY #Finaldata.Sdate,#Finaldata.Shiftid,#Finaldata.ShiftName
		ORDER BY #Finaldata.Sdate,#Finaldata.Shiftid



UPDATE #Summary SET QualityEfficiency= ISNULL(#Summary.QualityEfficiency,0) + IsNull(T1.QE,0)   
FROM(Select Sdate,  Shiftid,
CAST((Sum(Components))As Float)/CAST((Sum(IsNull(Components,0))+Sum(IsNull(RejCount,0))) AS Float)As QE  
From #Summary Where Components<>0 Group By Sdate,  Shiftid
)AS T1 Inner Join #Summary ON  #Summary.Sdate=T1.Sdate  
and #Summary.Shiftid =T1.Shiftid
...
select DISTINCT 
		ROUND(ProductionEfficiency,0) ProductionEfficiency,
		ROUND(AvailabilityEfficiency,0) AvailabilityEfficiency,
		ROUND(QualityEfficiency,0) as QualityEfficiency,
		ROUND(OverallEfficiency,0) OverallEfficiency,
		dbo.f_formattime(PlannedProductionTime,'hh') PlannedProductionTime,
		dbo.f_formattime(DownTime,@timeformat) as DownTime,
		dbo.f_formattime(UtilisedTime,@timeformat) as UtilisedTime, 
		dbo.f_formattime(isnull(TargetCycleTime,0),@timeformat) as CycleTime,
		dbo.f_formattime(isnull(LoadUnloadTime,0),@timeformat) as LoadUnload,
		dbo.f_formattime(isnull(PartCycleTime,0),@timeformat) as PartCycleTime,
		isnull(Round(OperationCount,2),0)as OperationCount, 
		cyclefficiency,LoadUnloadefficiency,
		dbo.f_formattime(D1,'mm') as D1,
		dbo.f_formattime(D2,'mm') as D2,
		dbo.f_formattime(D3,'mm') as D3,
		dbo.f_formattime(D4,'mm') as D4,
		dbo.f_formattime(D5,'mm') as D5,
		dbo.f_formattime(D6,'mm') as D6,
		dbo.f_formattime(D7,'mm') as D7,
		dbo.f_formattime(D8,'mm') as D8,
		dbo.f_formattime(D9,'mm') as D9,
		dbo.f_formattime(D10,'mm') as D10,
		dbo.f_formattime(D11,'mm') as D11,
		dbo.f_formattime(D12,'mm') as D12,
		dbo.f_formattime(D13,'mm') as D13,
		dbo.f_formattime(Others,'mm') as Others,
		dbo.f_formattime(TotalLoss,'mm') as TotalLoss,
		dbo.f_formattime(ActLoss,'mm') as ActLoss,
		dbo.f_formattime(LossErr,'mm') as LossErr,
		TargetCount as QuantityPlanned, 
		components as QuantityProduced,
		RejCount as QuantityRejected,
		QuantityOk as QuantityOk,
		Sdate as Day,
		Shiftid,
		ShiftName as shift
	    from  #Summary 
		WHERE (ISNULL(UtilisedTime,0) <> 0 OR ISNULL(DownTime,0) <> 0) 
		ORDER BY Sdate,Shiftid
--------------------------------------------------------------------------------------------------
-------------------------------------Simplee Avg---------------------------------------------------
SELECT DISTINCT 
		ROUND(AVG(#Summary.ProductionEfficiency),0) ProductionEfficiency,
		ROUND(AVG(#Summary.AvailabilityEfficiency),0) AvailabilityEfficiency,
		ROUND(AVG(#Summary.QualityEfficiency),0) as QualityEfficiency,
		ROUND(AVG(#Summary.OverallEfficiency),0) OverallEfficiency,
		dbo.f_formattime(SUM(#Summary.PlannedProductionTime),'hh') PlannedProductionTime,
		dbo.f_formattime(SUM(#Summary.DownTime),@timeformat) as DownTime,
		dbo.f_formattime(SUM(#Summary.UtilisedTime),@timeformat) as UtilisedTime, 
		dbo.f_formattime(isnull(SUM(#Summary.TargetCycleTime),0),@timeformat) as CycleTime,
		dbo.f_formattime(isnull(SUM(#Summary.LoadUnloadTime),0),@timeformat) as LoadUnload,
		dbo.f_formattime(isnull(SUM(#Summary.PartCycleTime),0),@timeformat) as PartCycleTime,
		isnull(Round(SUM(#Summary.OperationCount),2),0)as OperationCount, 
		cyclefficiency =
		SUM(CASE
		   when ( isnull(#Summary.TargetCycleTime,0) > 0 and
			  isnull(#Summary.ActualCycleTime,0) > 0
			) then (#Summary.TargetCycleTime/#Summary.ActualCycleTime)*100
		   else 0
		END),
		LoadUnloadefficiency =
		SUM(CASE
		   when ( isnull(#Summary.LoadUnloadTime,0) > 0 and
			  isnull(#Summary.ActualLoadUnload,0) > 0
			) then (#Summary.LoadUnloadTime/#Summary.ActualLoadUnload)*100
		   else 0
		END),
		dbo.f_formattime(isnull(SUM(#Summary.D1),0),'mm') as D1,
		dbo.f_formattime(isnull(SUM(#Summary.D2),0),'mm') as D2,
		dbo.f_formattime(isnull(SUM(#Summary.D3),0),'mm') as D3,
		dbo.f_formattime(isnull(SUM(#Summary.D4),0),'mm') as D4,
		dbo.f_formattime(isnull(SUM(#Summary.D5),0),'mm') as D5,
		dbo.f_formattime(isnull(SUM(#Summary.D6),0),'mm') as D6,
		dbo.f_formattime(isnull(SUM(#Summary.D7),0),'mm') as D7,
		dbo.f_formattime(isnull(SUM(#Summary.D8),0),'mm') as D8,
		dbo.f_formattime(isnull(SUM(#Summary.D9),0),'mm') as D9,
		dbo.f_formattime(isnull(SUM(#Summary.D10),0),'mm') as D10,
		dbo.f_formattime(isnull(SUM(#Summary.D11),0),'mm') as D11,
		dbo.f_formattime(isnull(SUM(#Summary.D12),0),'mm') as D12,
		dbo.f_formattime(isnull(SUM(#Summary.D13),0),'mm') as D13,
		dbo.f_formattime(isnull(SUM(#Summary.Others),0),'mm') as Others,
		dbo.f_formattime(isnull(SUM(#Summary.TotalLoss),0),'mm') as TotalLoss,
		dbo.f_formattime(isnull(SUM(#Summary.ActLoss),0),'mm') as ActLoss,
		dbo.f_formattime(isnull(SUM(#Summary.LossErr),0),'mm') as LossErr,
		isnull(SUM(#Summary.TargetCount),0) as QuantityPlanned, 
		isnull(SUM(#Summary.components),0) as QuantityProduced,
		isnull(SUM(#Summary.RejCount),0) as QuantityRejected,
		isnull(SUM(#Summary.QuantityOk),0) as QuantityOk
	    from  #Summary 
		WHERE (ISNULL(UtilisedTime,0) <> 0 OR ISNULL(DownTime,0) <> 0) 
--------------------------------------------------------------------------------------------------
-------------------------------------Weighted Avg---------------------------------------------------
...
FROM(Select 
CAST((Sum(Components))As Float)/CAST((Sum(IsNull(Components,0))+Sum(IsNull(RejCount,0))) AS Float)As QE  
From #Summary Where Components<>0 
)AS T1 
