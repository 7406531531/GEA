/****** Object:  Procedure [dbo].[s_GetShiftwiseMCOOLevelProdDownReport]    Committed by VersionSQL https://www.versionsql.com ******/

/************************************************************************************************************  
NR0122- swathiKS - 30/Jan/2016 :: To show Shiftwise M-C-O-O Level Utilisedtime, Downtime and Components for pari. (Implemented Batching Concept)  
--[dbo].[s_GetShiftwiseMCOOLevelProdDownReport]   '2016-04-26 07:00:00 AM','2016-04-26 03:30:00 PM','first','AWEA 6025','','','machinewiseDetails'
**************************************************************************************************************/  
CREATE PROCEDURE [dbo].[s_GetShiftwiseMCOOLevelProdDownReport]  
 @StartDate datetime,  
 @EndDate datetime,  
...
Select top 16 downid from downcodeinformation where catagory not in('Management Loss')
and SortOrder<=16 and SortOrder IS NOT NULL order by sortorder
...
---Get the down times which are not of type Management Loss  
 UPDATE #FinalTarget SET IgnoreDowns = isnull(IgnoreDowns,0) + isNull(t2.down,0)  
 from  
 (select  F.BatchStart,F.BatchEnd,F.machineinterface,F.Compinterface,F.Opninterface,F.oprinterface,  
  sum (CASE  
    WHEN (autodata.msttime >= F.BatchStart  AND autodata.ndtime <=F.BatchEnd)  THEN autodata.loadunload  
...
If (SELECT ValueInText From CockpitDefaults Where Parameter ='Ignore_Dtime_4m_PLD')='Y'  
BEGIN   
	 UPDATE  #FinalTarget SET IgnoreDowns = isnull(IgnoreDowns,0) - isNull(T2.PPDT ,0)  
	 FROM(  
	 SELECT F.BatchStart,F.BatchEnd,F.machineinterface,F.Compinterface,F.Opninterface,F.oprinterface,  
		SUM  
		(CASE  
...
END
 ...
 UPDATE #FinalTarget  set TotalAvailabletime = isnull(Utilisedtime,0) + isnull(downtime,0) --Datediff(s,BatchStart,BatchEnd) - isnull(ManagementLoss,0)




If @param = 'MachinewiseDetails'  or @param = ''
BEGIN  
 select Machineid,Convert(nvarchar(10),FromTm,120) as PDate,shift,Operator,Component,Operation,
 dbo.f_formattime(Stdtime,'hh:mm:ss') as Stdtime,Isnull(Round(sum(Components),2),0) as Components,
 dbo.f_formattime(sum(avgCycletime),'hh:mm:ss') as ActualCycletime,dbo.f_formattime(sum(Utilisedtime),'hh:mm:ss') as TotalProduction,
 dbo.f_formattime(sum(Downtime),'hh:mm:ss') as TotalDowntime  
,dbo.f_formattime(sum(TotalAvailabletime),'hh:mm:ss') as TotalAvailabletime
 ,dbo.f_formattime(sum(A),'hh:mm:ss') as A,dbo.f_formattime(sum(B),'hh:mm:ss') as B,dbo.f_formattime(sum(C),'hh:mm:ss') as C,dbo.f_formattime(sum(D),'hh:mm:ss') as D,
dbo.f_formattime(sum(E),'hh:mm:ss') as E,dbo.f_formattime(sum(F),'hh:mm:ss') as F,dbo.f_formattime(sum(G),'hh:mm:ss') as G,dbo.f_formattime(sum(H),'hh:mm:ss') as H,dbo.f_formattime(sum(I),'hh:mm:ss') as I,
dbo.f_formattime(sum(J),'hh:mm:ss') as J,dbo.f_formattime(sum(K),'hh:mm:ss') as K,dbo.f_formattime(sum(L),'hh:mm:ss') as L,dbo.f_formattime(sum(M),'hh:mm:ss') as M,
dbo.f_formattime(sum(N),'hh:mm:ss') as N,dbo.f_formattime(sum(O),'hh:mm:ss') as O,dbo.f_formattime(sum(P),'hh:mm:ss') as P from #FinalTarget  
...
If @param = 'OperatorwiseDetails'  
BEGIN  
 select operator,Convert(nvarchar(10),FromTm,120) as PDate,shift,machineid,Component,Operation,
 dbo.f_formattime(Stdtime,'hh:mm:ss') as Stdtime,Isnull(Round(sum(Components),2),0) as Components,
 dbo.f_formattime(sum(avgCycletime),'hh:mm:ss') as ActualCycletime,dbo.f_formattime(sum(Utilisedtime),'hh:mm:ss') as TotalProduction,
 dbo.f_formattime(sum(Downtime),'hh:mm:ss') as TotalDowntime  
,dbo.f_formattime(sum(TotalAvailabletime),'hh:mm:ss') as TotalAvailabletime
