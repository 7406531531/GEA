...
/*************************************************  
exec s_GetProcessDataString 'START-22-203-[63.5BASEREWOR]-1-552-6-20200413-153809-END','127.0.0.1','','33'  
exec s_GetProcessDataString 'START-41-203-3-20200427-190000-END','127.0.0.1','','33'  
...
---ER0421 added Till here  
 ...
 Declare @MachineEvent as Nvarchar(50) --NR0118  
  ...
  Declare @RunningMachine as nvarchar(50),@Runningcomponent as nvarchar(50),@RunningOpn as nvarchar(50) --FOR GEA
...
If  @tp=1 or @tp=2 OR @tp = 42 OR @tp = 11 OR  @tp = 22 or @tp=44-- ER0094  
BEGIN
...
Select @RunningMachine=Machineid from machineinformation where InterfaceID=@McInterfaceID
select @Runningcomponent=componentid from componentinformation  where interfaceid=@component
select @RunningOpn=operationno from componentoperationpricing where machineid=@RunningMachine and componentid=@Runningcomponent and InterfaceID=@operation
...
where DataType=1 and M.InterfaceID=@McInterfaceID and C.InterfaceID=@component and O.InterfaceID=@operation and R.WorkOrderNumber=@WorkOrder
and sttime>=(select max(sttime) from rawdata where DataType=11 and mc= @McInterfaceID and comp=@component and opn= @operation and WorkOrderNumber = @WorkOrder))
Begin
	...
	where MachineID=@RunningMachine and MaterialID=@Runningcomponent and OperationNo=@RunningOpn and ProductionOrder=@WorkOrder)T on T.ParkedTS=P.ParkedTS 
	where MachineID=@RunningMachine and MaterialID=@Runningcomponent and OperationNo=@RunningOpn and ProductionOrder=@WorkOrder and P.SelectedForRunning=1)
	BEGIN
		Delete From RunningScheduleDetails_GEA where MachineID=@RunningMachine and MaterialID=@Runningcomponent and OperationNo=@RunningOpn and ProductionOrder=@WorkOrder
		Delete From MO where Machineid=@RunningMachine and PartID=@Runningcomponent and OperationNo=@RunningOpn and MONumber=@WorkOrder --Added on 26/07/2021
	END
	ELSE
	BEGIN
		If Exists(SELECT * FROM ParkedScheduleDetails_GEA P inner join(SELECT MAX(PARKEDTS) as ParkedTS FROM ParkedScheduleDetails_GEA 
			where MachineID=@RunningMachine and MaterialID=@Runningcomponent and OperationNo=@RunningOpn and ProductionOrder=@WorkOrder)T on T.ParkedTS=P.ParkedTS 
			where MachineID=@RunningMachine and MaterialID=@Runningcomponent and OperationNo=@RunningOpn and ProductionOrder=@WorkOrder and P.SelectedForRunning=2)
		BEGIN

			If Exists(Select * from FinalInspectionTransaction_GEA F where MachineID=@RunningMachine and MaterialID=@Runningcomponent and OperationNo=@RunningOpn and ProductionOrderNo=@WorkOrder)
			Begin
				update ScheduleDetails_GEA set Status='Completed' where MachineID=@RunningMachine and MaterialID=@Runningcomponent and OperationNo=@RunningOpn and ProductionOrder=@WorkOrder
				update ScheduleDetails_GEA set ActualStarttime=@startdate+ ' ' + @starttime,ActualEndtime=@enddate+ ' ' + @endtime,TentativeEndtime=(Select TentativeEndtime from RunningScheduleDetails_GEA where MachineID=@RunningMachine and MaterialID=@Runningcomponent and OperationNo=@RunningOpn and ProductionOrder=@WorkOrder) where MachineID=@RunningMachine and MaterialID=@Runningcomponent and OperationNo=@RunningOpn and ProductionOrder=@WorkOrder
				Delete From RunningScheduleDetails_GEA where MachineID=@RunningMachine and MaterialID=@Runningcomponent and OperationNo=@RunningOpn and ProductionOrder=@WorkOrder
				Delete From MO where Machineid=@RunningMachine and PartID=@Runningcomponent and OperationNo=@RunningOpn and MONumber=@WorkOrder --Added on 26/07/2021
				update ParkedScheduleDetails_GEA set SelectedForRunning=3 where MachineID=@RunningMachine and MaterialID=@Runningcomponent and OperationNo=@RunningOpn and ProductionOrder=@WorkOrder
			End		
			Else
			Begin

				update ScheduleDetails_GEA set Status='Pending Inspection Completion' where MachineID=@RunningMachine and MaterialID=@Runningcomponent and OperationNo=@RunningOpn and ProductionOrder=@WorkOrder
				update ScheduleDetails_GEA set ActualStarttime=@startdate+ ' ' + @starttime,ActualEndtime=@enddate+ ' ' + @endtime,TentativeEndtime=(Select TentativeEndtime from RunningScheduleDetails_GEA where MachineID=@RunningMachine and MaterialID=@Runningcomponent and OperationNo=@RunningOpn and ProductionOrder=@WorkOrder) where MachineID=@RunningMachine and MaterialID=@Runningcomponent and OperationNo=@RunningOpn and ProductionOrder=@WorkOrder
				Delete From RunningScheduleDetails_GEA where MachineID=@RunningMachine and MaterialID=@Runningcomponent and OperationNo=@RunningOpn and ProductionOrder=@WorkOrder
				--Delete From MO where Machineid=@RunningMachine and PartID=@Runningcomponent and OperationNo=@RunningOpn and MONumber=@WorkOrder --Added on 26/07/2021
				update ParkedScheduleDetails_GEA set SelectedForRunning=3 where MachineID=@RunningMachine and MaterialID=@Runningcomponent and OperationNo=@RunningOpn and ProductionOrder=@WorkOrder
...
BEGIN
	...
	If not Exists(Select * from FinalInspectionTransaction_GEA where MachineID=@RunningMachine and MaterialID=@Runningcomponent and OperationNo=@RunningOpn and ProductionOrderNo=@WorkOrder)
	Begin
		update ScheduleDetails_GEA set Status='Pending Inspection Completion' where MachineID=@RunningMachine and MaterialID=@Runningcomponent and OperationNo=@RunningOpn and ProductionOrder=@WorkOrder
		update ScheduleDetails_GEA set ActualStarttime=@startdate+ ' ' + @starttime,ActualEndtime=@enddate+ ' ' + @endtime,TentativeEndtime=(Select TentativeEndtime from RunningScheduleDetails_GEA where MachineID=@RunningMachine and MaterialID=@Runningcomponent and OperationNo=@RunningOpn and ProductionOrder=@WorkOrder)
		where MachineID=@RunningMachine and MaterialID=@Runningcomponent and OperationNo=@RunningOpn and ProductionOrder=@WorkOrder
	End 
END

IF EXISTS(Select * From ScheduleDetails_GEA Where Status='Completed' and MachineID=@RunningMachine and MaterialID=@Runningcomponent and OperationNo=@RunningOpn and ProductionOrder=@WorkOrder)
BEGIN
	IF exists(select * from componentoperationpricing Where MachineID=@RunningMachine and componentid=@Runningcomponent and OperationNo=@RunningOpn and FinishedOperation=1)
	Begin
		...
		Select @SchedulePriority=ISNULL(Max(SchedulePriority),0),@UserPriority=ISNULL(MAX(UserPriority),0) from ScheduleDetails_GEA where Machineid='Quality In house'
		...
		from ScheduleDetails_GEA where MachineID=@RunningMachine and MaterialID=@Runningcomponent and OperationNo=@RunningOpn and ProductionOrder=@WorkOrder 
		and Status='Completed'

		IF NOT EXISTS(Select * From ScheduleDetails_GEA Where MachineID='Quality In house' and MaterialID=@Runningcomponent and OperationNo='60' and ProductionOrder=@WorkOrder)
		BEGIN
			...
			Select 'Quality In house', @WorkOrder, @Runningcomponent, 60, 1,'New',ISNULL(@SchedulePriority,1)+1, 'Service', getdate(),ISNULL(@UserPriority,1)+1, 
			...
			Select 'Quality In house', 1, @operator, getdate(), 0, getdate()
		END
	End
END
---For GEA
...
-- SET @datastring = SUBSTRING(@datastring,CHARINDEX('-', @datastring)+ 1,LEN(@datastring) - CHARINDEX('-', @datastring)+ 1)  
 ...
 update ScheduleDetails_GEA set Status='Running' where Machineid=@RunningMachine and Materialid=@Runningcomponent and OperationNo=@RunningOpn and ProductionOrder=@workorder and status in('New','Parked')

 Insert into  RunningScheduleDetails_GEA(Machineid, ProductionOrder, MaterialID, OperationNo)
 select Machineid, ProductionOrder, MaterialID, OperationNo from ScheduleDetails_GEA where Machineid=@RunningMachine and Materialid=@Runningcomponent and OperationNo=@RunningOpn and ProductionOrder=@workorder
 and Status='Running'
 --For GEA
 ...
 begin  
  ...
  --select @rejrecordid = isnull(max(recordid),0) from autodatarejections --ER0349 Commented  
   ...
   BEGIN  
    ...
    Select @RejectedTS = @RejDate +' '+ @starttime  
   ...
   if (SUBSTRING( @datastring,1,1)) = '['
	Begin
	 ...
	 -- SET @datastring = SUBSTRING(@datastring,CHARINDEX('-[', @datastring)+ 2,LEN(@datastring) - CHARINDEX('-[', @datastring)+ 2)  
	  SET @datastring = SUBSTRING(@datastring,CHARINDEX('[', @datastring)+ 1,LEN(@datastring) - CHARINDEX('[', @datastring)+ 1)  
	  --SET @Dimension = convert(nvarchar(20),SUBSTRING(@datastring,1,CHARINDEX(']-',@datastring) - 1))  
	  SET @Dimension = cast(convert(nvarchar(20),SUBSTRING(@datastring,1,CHARINDEX(']-',@datastring) - 1)) as decimal(5,3)) --g:
     -- SET @datastring = SUBSTRING(@datastring,CHARINDEX(']-', @datastring)+ 2,LEN(@datastring) - CHARINDEX(']-', @datastring)+ 2)
	  SET @datastring = SUBSTRING(@datastring,CHARINDEX(']', @datastring)+ 1,LEN(@datastring) - CHARINDEX(']', @datastring)+ 1)
...
End 
  ...
  If (IsDate(@startdate + ' ' + @starttime) = 1)  
  BEGIN  
   Insert into Rawdata(datatype,IPAddress,Mc,Comp,Opn,Opr,SPLSTRING3,Sttime,SPLSTRING2,Status,SPLSTRING4,SPLSTRING5,SPLSTRING6)values  
   (@tp_int,@IpAddress,@McInterfaceID,@component,@operation,@operator,@Dimension,@startdate+ ' ' + @starttime,@DimValue,0,@WearOffSetNumber,@MeasureDimension,@CorrectionValue)  
  END  
...
--START-77-202-20140329-23595901-2-4.00615-15.4385-1-[-0.56]-0.45-3-3-100-200-300-1-END  
*******************************************************************************************************/  
If @tp_int=77  
Begin  
  --startdate  
 ...
 -- SET @datastring = SUBSTRING(@datastring,CHARINDEX(']-', @datastring)+ 2,LEN(@datastring) - CHARINDEX(']-', @datastring)+ 2)  
  ...
  Begin Transaction  
   ...
   select @count = count(machineid) from tcs_energyconsumption where machineid=@MachineName  
	...
	If (@count = 0) or  (@gtime < @startdate + ' ' + @starttime)  
	begin  
		insert into dbo.tcs_energyconsumption (MachineID,gtime,ampere,watt,pf,KWH,Volt1,Volt2,Volt3,KVA,AmpereR,AmpereY,AmpereB, EnergySource)  
		values (@MachineName,@startdate + ' ' + @starttime,0.0,Round(@KWValue,5),Round(@PFValue,2),Round(@KWHVal,5),@V1,@V2,@V3,round(@KVA/1000,2),@AmpereR,@AmpereY,@AmpereB, @energysrc)   
		SET @Error = @@ERROR  
...
--SET @datastring = SUBSTRING(@datastring,CHARINDEX('-', @datastring)+ 1,LEN(@datastring) - CHARINDEX('-', @datastring)+ 1)  
 ...
 (@tp_int,@IpAddress,@McInterfaceID,@component,@operation,@operator,@startdate+ ' ' + @starttime,@downcode,@WorkOrder,0)  
