...
[s_GetEnergyData] '2019-01-01','','JYOTI','','Shift','','TechnoDashboard'      
*****************************************************************************************************************/      
...
CREATE procedure [dbo].[s_GetEnergyData]      -- testing for multiple shifts, using kwh1 and gtime1
 ...
 @HistoryLive nvarchar(50)='History', --History,Live, --NR0117      
...
Begin      
 ...
 Begin      
  ...
  Begin      
   ...
   select Machineinformation.MachineID,dbo.f_FormatTime(Sum(prodTime),'HH:MM:SS') as UtilisedTime,sum(pcount) as Components,Round(Avg(PF),2) as PF,Round(sum(Cost),2) as Cost,(Round(sum(Energy),2)) as  Energy from EnergyCockpit      
   inner join Machineinformation on machineinformation.MachineID = EnergyCockpit.Machineid      
   inner join PlantMachine on Plantmachine.Machineid = Machineinformation.MachineID      
   where PlantMachine.Machineid = @machineid and EnergyCockpit.dDate = @dDate      
   and machineinformation.MachineID in (select distinct MachineID from EnergyCockpit)      
  ...
  Begin      
   ...
   Begin      
    ...
    inner join Machineinformation on machineinformation.MachineID = EnergyCockpit.Machineid      
    inner join PlantMachine on Plantmachine.Machineid = Machineinformation.MachineID      
    where PlantMachine.PlantId = @PlantID and EnergyCockpit.dDate = @dDate      
    and machineinformation.MachineID in (select distinct MachineID from EnergyCockpit)      
    group by Machineinformation.MachineID      
   End      
   Else      
   Begin      
    ...
    inner join Machineinformation on machineinformation.MachineID = EnergyCockpit.Machineid      
    inner join PlantMachine on Plantmachine.Machineid = Machineinformation.MachineID      
    Where EnergyCockpit.dDate = @dDate and machineinformation.MachineID in (select distinct MachineID from EnergyCockpit)      
    group by Machineinformation.MachineID      
   End      
  End      
 End      
...
insert into #GetShiftTime Exec s_GetShiftTime @dDate,@Shift      
 ...
 Begin      
  ...
  Begin      
   ...
   Select PlantMachine.Machineid,Machineinformation.Interfaceid,0,      
  ...
  Begin      
   ...
   Begin      
    insert into #FinalData(MachineID,MachineInterface,ShiftHourID,StartTime,EndTime,UtilisedTime,components,PF,Cost,Energy,Maxenergy,Minenergy)      
    select Machineinformation.Machineid,Machineinformation.Interfaceid,0,      
    (select min(StartTime) from #GetShiftTime),(select max(EndTime) from #GetShiftTime),0,0,0,0,0,0,0 --NR0117      
    from Machineinformation inner join PlantMachine on machineinformation.Machineid = Plantmachine.Machineid      
...
--  order by Shiftdetails.ShiftID      
  ...
  select * from #GetShiftTime --rem
   insert into #FinalData(MachineID,MachineInterface,ShiftHourID,StartTime,EndTime,UtilisedTime,components,PF,Cost,Energy,Maxenergy,Minenergy)      
   Select PlantMachine.Machineid,Machineinformation.Interfaceid,#GetShiftTime.ShiftName,      
  ...
  Begin      
   ...
   select * from #GetShiftTime --rem    
    insert into #FinalData(MachineID,MachineInterface,ShiftHourID,StartTime,EndTime,UtilisedTime,components,PF,Cost,Energy,Maxenergy,Minenergy)      
    select Machineinformation.Machineid,Machineinformation.Interfaceid,#GetShiftTime.ShiftName,      
    #GetShiftTime.StartTime,#GetShiftTime.EndTime,0,0,0,0,0,0,0      
    from Machineinformation cross join #GetShiftTime       
    inner join PlantMachine on machineinformation.Machineid = Plantmachine.Machineid      
    where Plantmachine.PlantId = @PlantID and machineinformation.devicetype=5      
 ...
 --select 'hour' meh   
  insert into #FinalData(MachineID,MachineInterface,ShiftHourID,StartTime,EndTime,UtilisedTime,components,PF,Cost,Energy,Maxenergy,Minenergy)      
  select machineinformation.MachineID,machineinformation.interfaceid,HourName,      
...
round(AmpereR,2) as A1,Round(AmpereY,2) as A2,Round(AmpereB,2) as A3,Round(KVA,0) as KVA,Round(watt,0) as KW,      
maxgtime as LastArrival,round(PF,2) as PF,Round(KWH,0) as KWH from       
 (      
